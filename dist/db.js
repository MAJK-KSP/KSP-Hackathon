...
const initDb = async () => {
  ...
  `);
    ...
    await (0, exports.runQuery)(`CREATE INDEX IF NOT EXISTS idx_users_email ON users(email);`);
  ...
  `);
    await (0, exports.runQuery)(`CREATE INDEX IF NOT EXISTS idx_briefings_date ON daily_briefings(effective_date);`);
    // Chat conversations
    await (0, exports.runQuery)(`
  ...
  `);
    await (0, exports.runQuery)(`CREATE INDEX IF NOT EXISTS idx_messages_conversation ON chat_messages(conversation_id, created_at);`);
    // AI dataset registry
    await (0, exports.runQuery)(`
  ...
  `);
    await (0, exports.runQuery)(`CREATE INDEX IF NOT EXISTS idx_cases_crime_type ON cases(crime_type);`);
    await (0, exports.runQuery)(`CREATE INDEX IF NOT EXISTS idx_cases_police_station ON cases(police_station);`);
    await (0, exports.runQuery)(`CREATE INDEX IF NOT EXISTS idx_cases_date ON cases(reported_date);`);
...
const seedCases = async () => {
    try {
        const row = await (0, exports.getRow)('SELECT COUNT(*) as count FROM cases');
        if (row && row.count > 0) {
        ...
        for (const c of mockCases) {
            await (0, exports.runQuery)(`INSERT INTO cases (id, case_number, crime_type, jurisdiction, police_station, landmark, latitude, longitude, reported_date, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [c.id, c.case_number, c.crime_type, c.jurisdiction, c.police_station, c.landmark, c.latitude, c.longitude, c.reported_date, c.status]);
        }
